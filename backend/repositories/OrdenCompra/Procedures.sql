...
BEGIN
	...
	BEGIN TRY
		...
		IF EXISTS(SELECT 1 FROM Establecimientos WHERE RIF = @RIF_est)
		BEGIN
		...
		IF EXISTS(SELECT  1 FROM OrdenesCompra WHERE nro_OC = @nuevoID)
		BEGIN
			--Registar Compra
			IF EXISTS(SELECT 1 FROM Productos WHERE id_producto = @id_producto)
			BEGIN
				INSERT INTO Compras(nro_compra, id_producto, cantidad_producto, precio_und) VALUES (@nuevoID,@id_producto,@cant_producto, @precio);
			END
			ELSE
			BEGIN
			...
			IF EXISTS(SELECT 1 FROM Proveedores WHERE RIF = @RIF_proveedor)
			BEGIN
