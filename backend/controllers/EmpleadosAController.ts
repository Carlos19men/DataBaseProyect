...
export class EmployeeAsigController {
    ...
    asignEmployee = async(req: Request, res: Response<EmployeeAsig | {message: string}>): Promise<void> => {
        const RIF = req.params.RIF;
        const id_servicio = parseInt(req.params.id_servicio, 10);
        const Ci_emp = req.params.Ci_emp;

        try{
            const result = await employeeAsigModel.asigEmployee(RIF, id_servicio, Ci_emp);

            if(!result || 'error' in result){
                res.status(404).json({message: "No se pudo asignar el empleado"});
                return;
            }
            ...
            console.error("Ha ocurrido un error", error);
            res.status(500).json({message: "Error al asignar el empleado"});
        }
    }

    unassignEmployee = async(req: Request, res: Response<EmployeeAsig | {message: string}>): Promise<void> => {
        const id_servicio = parseInt(req.params.id_servicio, 10);
        const Ci_emp = req.params.Ci_emp;
        ...
        try{
            ...
            if(!result || 'error' in result){
                res.status(404).json({message: "No se pudo desasignar el empleado"});
                return;
            }
            ...
            console.error("Ha ocurrido un error", error);
            res.status(500).json({message: "Error al desasignar el empleado"});
        }
    }

    getByEmployee = async(req: Request, res: Response<EmployeeAsig | {message: string}>): Promise<void> => {
        const RIF = req.params.RIF;
        const Ci_emp = req.params.Ci_emp;
    ...
    getEmployeesNotAssigned = async(req: Request, res: Response<EmployeeAsig[] | {message: string}>): Promise<void> => {
        const RIF = req.params.RIF;
        ...
        try{
            ...
            if(!result || 'error' in result){
                res.status(404).json({message: "No se encontraron empleados no asignados"});
                return;
            }
            ...
            console.error("Ha ocurrido un error", error);
            res.status(500).json({message: "Error al obtener los empleados no asignados"});
        }
    }
}